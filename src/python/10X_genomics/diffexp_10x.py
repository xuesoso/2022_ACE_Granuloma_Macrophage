#!/usr/bin/env python
# original source: https://github.com/10XGenomics/cellranger/blob/master/lib/python/cellranger/analysis/diffexp.py
#
...
def sseq_differential_expression(x, cond_a, cond_b, sseq_params, big_count=900,
    ...
    feature_expressed_a = np.squeeze(np.asarray((x_a > min_count).sum(axis=1) / np.shape(x_a)[1]))
    feature_expressed_b = np.squeeze(np.asarray((x_b > min_count).sum(axis=1) / np.shape(x_b)[1]))
    ...
    de_result = pd.DataFrame({
        ...
        'expressed_a': feature_expressed_a,
        'expressed_b': feature_expressed_b,
        'p': p_values,
        'adjusted-p': adj_p_values,
        # Introduce a pseudocount into log2(fold_change)
        'log2-fold': np.log2((1 + feature_sums_a) / (1 + size_factor_a)) - \
        np.log2((1 + feature_sums_b) / (1 + size_factor_b))
    })

    return de_result

def run(adata, groupby, layer=None, sseq_params=None, directory=None):
    ...
    if sseq_params is None:
        ...
        sys.stdout.flush()
        if layer:
            sseq_params = compute_sseq_params(adata.layers[layer].T)
        else:
            sseq_params = compute_sseq_params(adata.X.T)
    ...
    for i, cluster in enumerate(unique_clusters):
        ...
        print('Computing DE for cluster %s...' %cluster)
        sys.stdout.flush()

        if layer:
            de_result = sseq_differential_expression(
                adata.layers[layer].T, group_a, group_b, sseq_params)
        else:
            de_result = sseq_differential_expression(
                adata.X.T, group_a, group_b, sseq_params)
        de_result.index = adata.var_names
        de_result = de_result.sort_values(['adjusted-p', 'log2-fold'], ascending=[True, False])
        out[cluster] = de_result
        if directory:
            de_result.to_csv(os.path.join(directory, str(cluster).replace('/',
                '_')+'_nb.tsv.gz'), sep='\t', compression='gzip')
    return out

def convert_to_pandas(groupby, de, adata):
    clusters = adata.obs[groupby].values
    ...
    out = {}
    for i, cluster in enumerate(unique_clusters):
        diff_expression_header = ['Cluster %s Mean Counts' % (cluster),
                                   'Cluster %s Log2 fold change' % (cluster),
                                   'Cluster %s Adjusted p value' % (cluster)]
        out[cluster] = pd.DataFrame(de[:, i*3+(i+1)*3], columns=diff_expression_header, index=features)
    return out


def save_differential_expression_csv(clustering_key, de, base_dir,
                                        cluster_names = None,
